...
\begin{enumerate}[label=\alph*), left=0pt]
  \item \fs{} Obtener el espectro de frecuencias para $N = 3$:
    ...
    %insertar imagen de la onda muesteada, chupala

    La serie de fourier discreta esta definida como:
    \begin{equation}
    ...
    Para llevar a cabo esta sumatoria, es recomendable expandirla termino a termino, ya que si pasamos el coseno a su
    forma compleja, y usamos la simplificacion de la serie geometrica (\ref{s.geometrica}), todos los terminos terminan
    valiendo cero.
    \begin{align*}
    ...
    Teniendo la expresion de $a_k$, la obtencion de su espectro es tan simple como obtener su modulo y fase.\\
    Para el modulo, usamos la expresion:
    \begin{equation}
      \label{modulo}
  ...
  \item \fs{} Obtener el espectro de frecuencias para $N = 6$:
    ...
    %insertar imagen de la onda muesteada, chupala 2, la saga completa
    ...
    Para llevar a cabo esta sumatoria, nuevamente, es recomendable expandirla termino a termino. Resolviendo
    algebraicamente:
    ...
    No fuimos capaces de determinar donde estaba el error.
