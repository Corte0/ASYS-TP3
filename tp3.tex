...
\begin{enumerate}[label=\alph*), left=0pt]
  ...
  \item\fs{} Si $h_1[n] = h_2[n] = 3^{-n}\mu[n]$, $h_3[n] = \mu[n]$, $h_4[n] = 2^{-n}\mu[n]$:
    ...
    \begin{align*}
      y[n] &= \mu[n] * h[n]\\[6pt]
      y[n] &= \sum_{k \exists \mathbb{Z}} -\frac{11}{4} - \frac{5k}{4 \cdot 3^k} + \frac{9k}{4 \cdot 3^{(k+1)}} -
        \frac{9}{4 \cdot 3^{(k+1)}} + \frac{3}{2 \cdot 3^{(k+1)}} + \frac{2}{2^{(k+1)}} \cdot \mu[k] \cdot \mu[n-k]\\[6pt]
      y[n] &= \mu[n] \cdot \sum_{k=0}^{n} -\frac{11}{4} - \frac{5k}{4 \cdot 3^k} + \frac{9k}{4 \cdot 3 \cdot 3^k} -
        \frac{9}{4 \cdot 3 \cdot 3^k} + \frac{3}{2 \cdot 3 \cdot 3^k} + \frac{2}{2 \cdot 2^k}\\[6pt]
      y[n] &= \mu[n] \cdot \sum_{k=0}^{n} -\frac{11}{4} - \frac{5k}{4 \cdot 3^k} + \frac{3k}{4 \cdot 3^k} -
        \frac{3}{4 \cdot 3^k} + \frac{1}{2 \cdot 3^k} + \frac{1}{2^k}
    \end{align*}

    El desarrollo, ademas de largo, es engorroso y carece de la aplicacion de alguna propiedad importante o identidad
    no vista. Todos los terminos de la suma usan o la identidad de la serie trigonometrica, su derivada, o la identidad
    de una sumatoria constante. Como resultado, se obtiene lo siguiente:
    \begin{equation*}
      y[n] = \frac{5}{4} - \frac{11}{4} \cdot (n+1) + \frac{5n + 1}{8 \cdot 3^n} + \frac{3 \cdot (n+1)}{8 \cdot 3^n} -
        \frac{1}{2^n}
    \end{equation*}
