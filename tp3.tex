...
\begin{enumerate}[label=\alph*), left=0pt]
  ...
  \item \fs{} Obtener el espectro de frecuencias para $N = 6$:
    ...
    \[
      a_k = \frac{1}{3} \sum_{n=0}^{3-1} \left(1 + cos\left(\frac{\pi n}{3}\right) \right) \cdot e^{-j\frac{2 \pi k n}{N}}
    \]

    Para llevar a cabo esta sumatoria, nuevamente, es recomendable expandirla termino a termino, ya que si pasamos el coseno a su
    forma compleja, y usamos la simplificacion de la serie geometrica (\ref{s.geometrica}), todos los terminos terminan
    valiendo cero.
    \begin{align*}
